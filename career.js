...
(function() {
    ...
    function getLeagueClubs() {
        return ['Manchester United','Manchester City','Liverpool','Chelsea','Arsenal','Tottenham Hotspur','Newcastle United','Aston Villa','West Ham United','Everton','Fulham','Brighton & Hove Albion','Crystal Palace','Wolverhampton Wanderers','Nottingham Forest','Brentford','Bournemouth','Burnley','Leicester City','Leeds United','Southampton','Stoke City','Swansea City','Huddersfield Town','Norwich City'];
    }
    ...
    function generateNextMatch() {
        ...
        return {
            home: isHome ? player.club : opponent,
            away: isHome ? opponent : player.club,
            date: nextDate.toISOString(),
            league, leagueLogo,
            homeLogo: clubLogos[isHome ? player.club : opponent] || '',
            awayLogo: clubLogos[isHome ? opponent : player.club] || '',
    ...
    document.getElementById('btnSimulateMatch').addEventListener('click', function() {
        ...
        setTimeout(() => { displayNextMatch(); }, 5000);
    });

    // ==================== Handle Skip Match ====================
    document.getElementById('btnSkipMatch').addEventListener('click', function() {
        if (!player.nextMatch) { displayNextMatch(); return; }
        advanceWeek();
        player.careerTimeline.push({
            date: new Date().toISOString(),
            event: 'Skipped Week',
        ...
        updateCareerUI();
        setTimeout(() => { displayNextMatch(); }, 1000);
    });
    ...
    console.log('%c⚽ CAREER HUB LOADED (FIXED) %c| %c' + (player.name || 'Player') + ' %c| Week ' + (player.week || 1),
